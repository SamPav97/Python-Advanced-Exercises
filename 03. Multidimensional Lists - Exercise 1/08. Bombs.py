def is_inside(ro, co, sizze):
    return 0 <= ro < sizze and 0 <= co < sizze


def determine_neighbours(r, c, mat):
    neighs = []
    length = len(mat)

    if is_inside(r-1,c, length) and mat[r-1][c] > 0:
        neighs.append([r-1, c])
    if is_inside(r-1,c-1, length) and mat[r-1][c-1] > 0:
        neighs.append([r-1, c-1])
    if is_inside(r-1,c+1, length) and mat[r-1][c+1] > 0:
        neighs.append([r-1, c+1])
    if is_inside(r,c-1, length) and mat[r][c-1] > 0:
        neighs.append([r, c-1])
    if is_inside(r,c+1, length) and mat[r][c+1] > 0:
        neighs.append([r, c+1])
    if is_inside(r+1,c, length) and mat[r+1][c] > 0:
...
for bomb in bombs:
    ...
    strength = matrix[row][column]
    matrix[row][column] = 0
    for neighbour in determine_neighbours(row, column, matrix):
        nei_row, nei_column = [int(x) for x in neighbour]
        matrix[nei_row][nei_column] -= strength
